<template>
    <div>
        <div class="grid grid-cols-2 bg-light rounded-lg mb-9">
            <router-link class="flex p-4 gap-3 justify-center font-medium items-center rounded-lg" to="/login">

                <i class="dr dr-security-safe"></i>
                ورود
            </router-link>
            <router-link class="flex gap-3 justify-center font-medium items-center bg-primary-default rounded-lg text-white" to="/register">
                <i class="dr dr-user"></i>
                ثبت نام
            </router-link>
        </div>
        <h2 class="font-head text-xl font-bold text-center mb-4">ثبت نام</h2>
        <p class="text-center text-sm text-gray-700">به دراگون خوش اومدی! برای عضویت یکی از راهای زیر رو انتخاب کن</p>
        ...
        <div class="relative flex py-5 items-center">
            ...
            <span class="flex-shrink mx-4 text-gray-600 text-sm">یا ثبت نام با</span>
            <div class="flex-grow border-t border-gray-300"></div>
        </div>
        <form ref="form" @submit.prevent="login">
            <div class="flex flex-col gap-y-3 mb-6">
                <label class="font-semibold text-sm text-gray-800 pr-1" for="email">ایمیل</label>
                <input class="bg-light h-14 rounded-lg p-2 text-sm" type="text" name="user_name" tabindex="0" placeholder="ایمیل یا نام کاربری خود را وارد کنید">
            </div>
